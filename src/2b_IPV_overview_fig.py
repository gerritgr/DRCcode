...
def aggregate_variable(data, variable_name, use_weights=True):
    ...
    3. Uses DHS sampling weights (v005/1,000,000) if requested
    ...
    if use_weights:
        # Use DHS sampling weights (v005/1,000,000)
        # Create sampling_weight column if it doesn't exist
        if 'sampling_weight' not in data_with_response.columns:
            if 'v005' in data_with_response.columns:
                data_with_response['sampling_weight'] = (
                    pd.to_numeric(data_with_response['v005'], errors='coerce') / 1_000_000.0
                )
            else:
